...
interface Report {
  ...
  reason: string | null;
  status: string | null;
  proof: string | null; // URL của ảnh bằng chứng, không phải ID
}
...
export default function ReportsPage() {
  ...
  const [reportToDelete, setReportToDelete] = useState<Report | null>(null); // State mới để xác nhận xóa
  ...
  async function fetchReports() {
    try {
      ...
      const userIds = Array.from(
        new Set(
          reportsData
            .flatMap((r) => [r.reporter_id, r.targeted_user_id])
            .filter(Boolean)
        )
      ) as string[];
      const roomIds = Array.from(
        new Set(reportsData.map((r) => r.room_id).filter(Boolean))
      ) as string[];
      ...
      if (roomIds.length) {
        ...
        setRoomsMap(
          new Map((rooms ?? []).map((r) => [r.id, { name: r.title }]))
        );
      }
    } catch (err) {
      console.error(err);
  ...
  async function confirmDelete() {
    ...
    try {
      // 1. XÓA FILE BẰNG CHỨNG TRÊN STORAGE
      // Dựa vào route /api/proof DELETE, chúng ta chỉ cần reportId (vì nó tương ứng với folder)
      ...
      if (!deleteProofResponse.ok) {
        // Báo lỗi nhưng vẫn tiếp tục xóa DB để tránh record rác,
        // nhưng sẽ cảnh báo người dùng.
    ...
    <div className="p-6">
          ...
          columns={[
                    ...
                    content={
                      <div className="text-gray-900 dark:text-gray-100 max-w-xs sm:max-w-sm">
                        <p>
                          ...
                          {usersMap.get(r.reporter_id)?.name ?? "N/A"}
                        </p>
                        <p>
                          <strong className="text-gray-900 dark:text-gray-100">
                            Email:
                          </strong>{" "}
                          {usersMap.get(r.reporter_id)?.email ?? "N/A"}
                        </p>
                        <p>
                          <strong className="text-gray-900 dark:text-gray-100">
                            ID:
                ...
                r.targeted_user_id ? (
                    ...
                    content={
                      <div className="text-gray-900 dark:text-gray-100">
                        <p>
                          ...
                          {usersMap.get(r.targeted_user_id)?.name ?? "N/A"}
                        </p>
                        <p>
                          <strong className="text-gray-900 dark:text-gray-100">
                            Email:
                          </strong>{" "}
                          {usersMap.get(r.targeted_user_id)?.email ?? "N/A"}
                        </p>
                        <p>
                          <strong className="text-gray-900 dark:text-gray-100">
                            ID:
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        router.push(`pages/user/${r.targeted_user_id}`)
                      }
                    >
                      {usersMap.get(r.targeted_user_id)?.name ??
                        r.targeted_user_id}
                    </button>
                  </HoverCard>
                ) : (
                    ...
                    content={
                      <div className="text-gray-900 dark:text-gray-100">
                        <p>
                          ...
                          {roomsMap.get(r.room_id)?.name ?? "N/A"}
                        </p>
                        <p>
                          <strong className="text-gray-900 dark:text-gray-100">
                            Room ID:
            ...
            { key: "reason", label: "Reason", width: "w-[18 0px]" },
            {
              ...
              render: (r: Report) => {
                ...
                switch (value) {
                  case "reviewed":
                    return (
                      <span className="flex items-center gap-1 px-2 py-1 rounded-full bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 font-medium text-sm">
                        <span>✅</span> Reviewed
                      </span>
                    );
                  case "rejected":
                    return (
                      <span className="flex items-center gap-1 px-2 py-1 rounded-full bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 font-medium text-sm">
                        <span>❌</span> Rejected
                      </span>
                    );
                  default:
                    return (
                      <span className="flex items-center gap-1 px-2 py-1 rounded-full bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 font-medium text-sm">
                        <span>🟡</span> Pending
                      </span>
                    );
                }
            ...
            },
            {
              ...
              render: (r) =>
                r.proof ? (
                  <button
                    onClick={() => setPreviewImage(r.proof ?? null)}
                    className="p-1 border dark:border-gray-700 rounded-lg hover:shadow-md transition"
                  >
                    <img
                      src={r.proof}
                      alt="Proof"
      ...
      {reportToDelete && (
        <EditablePopup
          ...
          isDeleteAction={true} // Tùy chỉnh button Save thành Delete
          fields={[]}
        >
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Are you sure you want to delete report **{reportToDelete.id}**?
            <br />
            This action will also **permanently delete all associated proof
