...
export default function BookingHistory() {
  ...
  const fetchBookings = async (loadMore = false) => {
    ...
    setBookings((prev) => (loadMore ? [...prev, ...data] : data));
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-6 pt-32">
      ...
      {bookings.map((b) => (
        ...
        >
          ...
          <div className="p-4 space-y-2">
            ...
            <p>
              <strong>Address:</strong> {b.rooms?.address || "Not available"}
            </p>
            <p>
              <strong>City:</strong> {b.rooms?.city || "Not available"}
            </p>
            <p>
              <strong>Area:</strong> {b.rooms?.area || "Unknown"}
            </p>
            <p>
              <strong>Price:</strong>{" "}
              {b.rooms?.price
                ? b.rooms.price.toLocaleString("en-US") + "₫ / month"
                : "Unknown"}
            </p>
      ...
      {!hasMore && (
        <p className="text-center text-gray-500">No more bookings.</p>
      )}
